...
def defineGenres_subject(w, keywords, query_second_part, selected_genres):
    ...
    for k in selected_genres:
        gnr = ' BIND((IF (REGEX(lcase(xsd:string(?list)), ".' + k + '."), 15 , -10)) AS ?genre' + str(
            c_gnr) + '). '
...
def abstract_keyword(keywords, query_second_part, scorer):
    index = 0
    ...
    for k in keywords:
        penalties = 0
        weight = int(k[1] * 100)
        if k[0] in word_vector.vocab:
            similar = w2v_model.wv.most_similar(positive=[k[0]])
        else:
            similar = []
        if len(k[0]) > 2 and k[0] != 'film' and k[0] != 'movie':
            if len(similar) > 0:
                binder = ' BIND((IF (REGEX(lcase(xsd:string(?abstract)), ".' + k[0] + '."), ' + str(
                    weight) + ' , ' + str(penalties) + ')) AS ?' + k[
                             0].replace(
                    "-", "") + '). '
                binder_similar = ' BIND((IF (REGEX(lcase(xsd:string(?abstract)), ".' + similar[0][
                    0] + '."), ' + str(int(weight / 2)) + ' , ' + str(penalties) + ')) AS ?' + similar[
                                     0][0].replace(
                    "-", "") + '). '
                ...
                binder = ' BIND((IF (REGEX(lcase(xsd:string(?abstract)), ".' + k[
                    0].replace("-", "/W") + '."), ' + str(weight) + ' , ' + str(penalties) + ')) AS ?' + k[
                             0].replace(
                    "-", "") + '). '
...
def subject_keyword(keywords, query_second_part):
    ...
    for k in keywords:
        ...
        binder = ' BIND((IF  (regex(lcase(xsd:string(?list)), ".' + k[
            0].replace('-', '/W') + '."),  ' + str(weight) + ' , ' + str(penalties) + ')) AS ?special' + str(
            s1) + ' ).  '
        query_second_part = query_second_part + binder
        if count == (len(keywords) - 1):
...
def queryConstructor(msg, Idf, language, year, no_genre, limit, english_dictionary):
    ...
    if len(keywords) >= 1:
        ...
        query_second_part, scorer = abstract_keyword(keywords, query_second_part, scorer)
        ...
        query_first_part = ' PREFIX rdfs:<http://www.w3.org/2000/01/rdf-schema#> ' \
                           ...
                           ' SELECT DISTINCT  ?top_kw ?score  ?id ?movie ?movie_title ?year1 ?abstract ?link ?list ?genre0  WHERE{ ' \
                           ' { SELECT DISTINCT  ?score1 ?top_kw ?id ?movie ?movie_title ?year1 ?abstract ?link (group_concat(distinct ?subj1; separator = " ") as ?list) ?genre0 ' \
